...
async function main() {
  ...
  const rawData: any = await prisma.mentorshipRequest.findRaw({
    filter: {
  ...
  if (!Array.isArray(rawData)) {
    console.error(
      "Error: Expected an array of requests but received something else."
    );
    return;
  }
  ...
  if (requestsToUpdate.length === 0) {
    console.log(
      "✅ No mentorship requests needed updating. Your data is clean!"
    );
    return;
  }
