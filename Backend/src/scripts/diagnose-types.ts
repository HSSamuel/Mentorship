  ...
  try {
    ...
    for (const session of sessions) {
      ...
      if (
        ...
        !(mentorId instanceof ObjectId)
      ) {
        isInvalid = true;
        console.error(
          `🔴 Invalid Document Found (ID: ${session._id}): mentorId is of type [${typeof mentorId}], not a string or ObjectId.`
        );
      }
      ...
      if (
        ...
        !(menteeId instanceof ObjectId)
      ) {
        isInvalid = true;
        console.error(
          `🔴 Invalid Document Found (ID: ${session._id}): menteeId is of type [${typeof menteeId}], not a string or ObjectId.`
        );
      }
    ...
    if (invalidCount > 0) {
      ...
      console.log(
        "If this script finds no errors, the problem is exceptionally unusual and may be related to the Prisma engine or database driver version itself."
      );
    }
  } catch (err) {
